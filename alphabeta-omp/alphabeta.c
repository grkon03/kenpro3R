...
void _alphabeta_parallel(gametree tree, int depth, int maxdepth, int alpha, int beta);
...
{
    // _clear_value(tree);
    _alphabeta_parallel(tree, 1, maxdepth, ABP_MINFTY, ABP_PINFTY);
}
...
#pragma omp parallel for
    for (i = 0; i < tree->no_leaves; ++i)
    {
        gametree searching = tree->leaves + i;
        _alphabeta(searching, depth + 1, maxdepth, alpha, beta);
